...
### 1. Mở khóa các tính năng nâng cao (Premium) phải mua ở bản thương mại và đóng gói sẵn trong file APK phát hành ở mục Releases

### 2. Sửa lỗi build APK trên GitHub Actions
...
* **Fix lỗi vòng đời build Gradle:** Khắc phục lỗi `Cannot run Project.afterEvaluate(Closure) when the pject is already evaluated` bằng việc kiểm tra trạng thái `project.state.executed` của các plugin trước khi gọi `afterEvaluate`.
...
### 3. Tự động tải file APK lên GitHub Releases
...
### 4. Sửa lỗi Runtime khi lưu Server AList mới
